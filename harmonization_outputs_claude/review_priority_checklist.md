...
- **Evidence:** Template GU16-257 BOR: Y=112, N=78, NE=6 (total 196). Source RESPTYPE alone gives ~57% match. Other response flags exist and may combine.
- **Question for reviewer:** How is GU16-257 BOR Y/N/NE derived from response.csv? Single column? Composite of CLCRFL + RECCUR?
- **Recommended next action:** Compute combinations of (RESPTYPE, CLCRFL, RECCUR) per row and find the rule that reproduces template BOR. Likely a 2-line Python check on the response file.
...
- **Evidence:** 196 cells in 10013 have confidence below threshold for BOR.binary.
- **Question for reviewer:** Is there a known authoritative source for 10013 BOR.binary that the pipeline missed?
- **Recommended next action:** Filter flagged_for_review.csv to trial=='10013' & harmonized_field=='BOR.binary', inspect notes / proposed_mapping; decide whether the field is genuinely unrecoverable from source (then accept NA) or the YAML config needs a new column / value_map.
...
- **Candidate source files:** (see flagged_for_review.csv source_files col)
- **Candidate source columns:** (see flagged_for_review.csv candidate_source_variables col)
- **Evidence:** 196 cells in 10013 have confidence below threshold for phase.
- **Question for reviewer:** Is there a known authoritative source for 10013 phase that the pipeline missed?
- **Recommended next action:** Filter flagged_for_review.csv to trial=='10013' & harmonized_field=='phase', inspect notes / proposed_mapping; decide whether the field is genuinely unrecoverable from source (then accept NA) or the YAML config needs a new column / value_map.
...
- **Evidence:** Validation: template_match_rate=0.748. Investigate specific rows via validation_report.csv filter trial=='10026' & column=='BOR.binary'.
- **Question for reviewer:** What source column or transformation should be used for 10026 BOR.binary to better match the template?
- **Recommended next action:** Inspect validation_report.csv for 10026/BOR.binary; identify the divergence pattern; refine value_map or column choice in harmonization_config.yaml; re-run pipeline.

### 10026 · `phase`

- **Issue type:** high_flag_volume
- **Rows affected:** 214
- **Current behavior:** 214 rows flagged for review (NA in harmonized output).
- **Candidate source files:** (see flagged_for_review.csv source_files col)
- **Candidate source columns:** (see flagged_for_review.csv candidate_source_variables col)
- **Evidence:** 214 cells in 10026 have confidence below threshold for phase.
...
- **Candidate source files:** (see provenance_long.csv for this (trial, field))
- **Candidate source columns:** (see provenance_long.csv)
- **Evidence:** Validation: template_match_rate=0.920. Investigate specific rows via validation_report.csv filter trial=='10104' & column=='BOR.binary'.
- **Question for reviewer:** What source column or transformation should be used for 10104 BOR.binary to better match the template?
- **Recommended next action:** Inspect validation_report.csv for 10104/BOR.binary; identify the divergence pattern; refine value_map or column choice in harmonization_config.yaml; re-run pipeline.
...
- **Candidate source files:** DERIVED from BOR.binary + pfs_time
- **Candidate source columns:** BOR.binary, pfs_time
- **Evidence:** Across 9 template trials, BOR.binary=SD splits 99/178 between bor_bin=0/1 (clearly not a function of BOR.binary alone). Mean pfs_time differs: 83d for bor_bin=0, 266d for bor_bin=1. Hypothesis `1 iff R OR (SD AND pfs_time>=183)` matches 86% of CIMAC-s1400i rows and 78% of EAY131_Z1D — close to the standard 'clinical benefit rate' definition but not perfect.
